...
#ifndef QPAINTENGINE_JSON_P_H
#define QPAINTENGINE_JSON_P_H
...
class QJsonPaintEnginePrivate;
class QBuffer;

class QJsonPaintEngine : public QPaintEngine
{
	Q_DECLARE_PRIVATE(QJsonPaintEngine)

	void setCanvasPen();
	void setCanvasFillBrush();

public:
	QJsonPaintEngine(QIODevice *buffer);
	~QJsonPaintEngine();
...
protected:
	QJsonPaintEngine(QJsonPaintEnginePrivate &dptr);

private:
	Q_DISABLE_COPY(QJsonPaintEngine)
};
